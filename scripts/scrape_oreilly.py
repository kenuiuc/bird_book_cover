...
def clean_animal_name(name: str) -> str:
        """Normalize animal name by removing a single trailing parenthetical group.

        Examples:
            'Scarlet Macaw (bird)' -> 'Scarlet Macaw'
            'Egret (Bird)' -> 'Egret'
            'Barn Owl (avian, bird)' (only last group) -> 'Barn Owl (avian, bird)'  # we only strip if entire tail is one (...)
        """
        if not name:
                return name
...
def extract_cover_title_animal(book_html: str, book_url: str):
    ...
    cover = None
    if og and hasattr(og, 'attrs'):
        try:
            cover = og.get('content')  # type: ignore[attr-defined]
        except Exception:
            cover = None
    if not cover:
...
def render_markdown(items, out_path):
    ...
    with p.open('w', encoding='utf8') as f:
        f.write('# O\'Reilly Media Bird Gallery\n\n')
        ...
        for it in items:
            ...
            cover = it.get('cover')
            book_url = it.get('url')
            animal = (it.get('animal') or '').strip() or None
            f.write('<div style="border:1px solid #ddd;padding:8px;border-radius:6px;background:#fff;">')
            # Image (alt prefers animal name; fallback to title)
            alt_txt = animal or title
            if cover:
                f.write(f'<a href="{book_url}" target="_blank" rel="noopener noreferrer">'
                        f'<img src="{cover}" alt="{alt_txt}" style="width:100%;height:260px;object-fit:cover;border-radius:4px;" loading="lazy"></a>')
            else:
            ...
            # Text block: ONLY bird (animal) name as a Wikipedia link
            f.write('<div style="margin-top:8px;">')
            if animal:
                wiki_title = animal.replace(' ', '_')
                wiki_url = f'https://en.wikipedia.org/wiki/{quote(wiki_title)}'
                f.write(
                    f'<a href="{wiki_url}" target="_blank" rel="noopener noreferrer" '
                    f'style="font-size:1.0rem;font-weight:600;line-height:1.15;color:#0366d6;text-decoration:underline;">{animal}</a>'
                )
            else:
                # No animal name: keep a small placeholder or leave blank; choose blank per request to remove book names
                f.write('<span style="font-size:0.75rem;color:#bbb;">(no bird name)</span>')
